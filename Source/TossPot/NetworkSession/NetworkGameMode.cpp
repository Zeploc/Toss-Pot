...
void ANetworkGameMode::BeginPlay()
{
	...
	UCommonFunctions::AddDebugMessage("Gamemode Created", false, bDebugMessages);
...
void ANetworkGameMode::Tick(float DeltaTime)
{
	...
	if (bDebugMessages)
	{
		...
		for (int i = 0; i < Controllers.Num(); i++)
		{
			...
			GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Black, Message);
		}
...
void ANetworkGameMode::SpawnPlayerPawn_Implementation(ANetworkController * NetworkController, FVector Location)
{
	...
	if (NetworkCharacter)
	{
		...
		FString PrintMessage = FString("CREATING PLAYER for controller ") + NetworkController->GetName() + FString(" called ") + NetworkCharacter->GetName();
		UCommonFunctions::AddDebugMessage(PrintMessage, false, bDebugMessages);
		/*if (bDebugMessages)
			GEngine->AddOnScreenDebugMessage(-1, 20.0f, FColor::Black, PrintMessage);*/
	}
	else
	{
		UCommonFunctions::AddDebugMessage(FString("COUDLN'T CREATE PLAYER for controller ") + NetworkController->GetName(), false, bDebugMessages);
		//if (bDebugMessages)
...
void ANetworkGameMode::SwapPlayerControllers(APlayerController * OldPC, APlayerController * NewPC)
{
	...
	return;
	if (ANetworkController* NetworkController = Cast<ANetworkController>(NewPC))
	{
		NetworkPlayerControllers.AddUnique(NetworkController);
		if (!ControllersComponentsReady.Contains(NetworkController))
		{
			ControllersComponentsReady.Add(NetworkController, TArray<UNetworkInitialiseComponent*>());
		}
